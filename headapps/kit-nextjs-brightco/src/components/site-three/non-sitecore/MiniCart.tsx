    ...
    <div ref={ref}>
      ...
      >
        <div className="pt-18 p-8 lg:pt-8">
          <h5 className="mb-4 uppercase">{getTranslation(DICTIONARY_KEYS.MINI_CART_LABEL, 'Your Cart')}</h5>
          <p className="mb-8">
            {getTranslation(DICTIONARY_KEYS.CART_EMPTY_LABEL, 'Your cart is currently empty.')}
          </p>
          <ContentSdkLink field={cartLink} className="btn btn-primary btn-sharp">
            {getTranslation(DICTIONARY_KEYS.GO_TO_CART_LABEL, 'Go to Cart')}
          </ContentSdkLink>
        </div>
      </div>
    </div>
