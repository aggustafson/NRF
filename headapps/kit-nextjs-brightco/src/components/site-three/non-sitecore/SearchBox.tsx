    ...
    <div ref={ref}>
      ...
      >
        <div className="pt-18 p-8 lg:pt-8">
          <h5 className="mb-4 uppercase">{getTranslation(DICTIONARY_KEYS.SEARCH_LABEL, 'Search')}</h5>
          <div className="flex gap-4">
            <input
              type="text"
              placeholder={getTranslation(DICTIONARY_KEYS.SEARCH_INPUT_PLACEHOLDER, 'Type to search...')}
            ...
            <Link href={buildSearchUrl()} prefetch={false} className="btn btn-primary btn-sharp">
              {getTranslation(DICTIONARY_KEYS.SEARCH_GO_LABEL, 'Go')}
            </Link>
          </div>
        </div>
      </div>
